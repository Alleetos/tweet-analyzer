...
interface SentimentAnalysisSummary {
  results: Tweet[];
}

const SentimentAnalysisSummary = ({ results }: SentimentAnalysisSummary) => {
  const totalTweets = results.length;
  const positiveTweets = results.filter(
    (r) => r.Sentiment === "positive"
  ).length;
  const negativeTweets = results.filter(
    (r) => r.Sentiment === "negative"
  ).length;
  const neutralTweets = results.filter((r) => r.Sentiment === "neutral").length;

  // Custom category counts
  const customFelicidade = results.filter(
    (r) => r.Custom_Category === "felicidade"
  ).length;
  const customTristeza = results.filter(
